...
import { CategoryResponseDto } from './dto/responses/category.reponse.dto';
...
export class CategoriesController {
  ...
  create(@Body() createCategoryDto: CreateCategoryDto) {
    return toDto(CategoryResponseDto, this.categoriesService.create(createCategoryDto)) 
  }
  ...
  findOne(@Param('id') id: string) {
    ...
    return  toDto(CategoryResponseDto, this.categoriesService.findOne(id)) 
  }
  ...
  async getAllPostByCategoryId(
    ...
    console.log('d', id);
    if (!isUUID(id)) throw new HttpException('Category id invalid', 400);
    const data = await this.categoriesService.getAllPostByCategoryId(
      id,
      paginationQuery,
    );
  ...
  update(
    ...
    return toDto(CategoryResponseDto,this.categoriesService.update(id, updateCategoryDto));
  }
